...
function sendToPrinter($printer_name, $data) {
    ...
    // Method 1: Try direct printer access via file write
    try {
        ...
        if ($handle) {
            fwrite($handle, $data);
            fclose($handle);
            return ['success' => true, 'message' => 'Printed via direct access', 'method' => 'direct'];
        }
    } catch (Exception $e) {
        // Continue to next method
    }

    // Method 2: Try using temp file and print command
    $temp_file = sys_get_temp_dir() . '\\kot_' . time() . '.txt';
    ...
    if ($return_var === 0) {
        return ['success' => true, 'message' => 'Printed via print command', 'method' => 'command'];
    }
